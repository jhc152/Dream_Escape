...
import { engine, GltfContainer, Transform, Animator, Schemas } from '@dcl/sdk/ecs'
...
function zombieMovementSystem(deltaTime: number) {
  ...
  for (const [entity] of engine.getEntitiesWith(Zombie)) {
    ...
    let isDmagedPlayer = distance < ATTACK_Player
    if(isInAttackDistance){
      console.log("atacando")
      DamagePlayer()
      isDmagedPlayer = false
...
export function RemoveMovementSystem(){
  engine.removeSystem(zombieMovementSystem)
}
